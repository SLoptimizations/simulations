...
def prepare_html(json, url_id):
    ...
    if json:
        html_details = json['email_content']
...
def send_mail(to, campaign_json, url_id):
    try:
        ...
        # getting content
        if c_json['html_file_name'] != "":
            # getting values into HTML
            content = prepare_html(c_json, url_id)
        else:
            content = c_json['email_content']['text']

        # sending the email
        yag.send(to=to, subject=c_json['email_subject'], contents=content)
